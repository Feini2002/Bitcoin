...
3. **GitHub 推送逻辑与脑内决策树（Agent 必须执行的判断）**  
   ...
   **步骤 1：判断目标分支 (Main vs Branch)**
   - **默认推 `main` 的条件**：本次改动范围小且明确、已完整完成 `.cursorrules` 与 `AGENTS.md` 要求的本地校验与部署、功能不破坏现有业务逻辑。
...
**异常退出机制**：若部署或校验在本机彻底失败，且不符合“阶段性保存”的诉求，必须向用户汇报失败原因；在未确认修复前不要强行合入 `main`，以免把明显坏的状态推上去。
